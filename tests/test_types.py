...
def testparse_bandcamp_date_returns_utc_datetime():
    ...
    assert dt == datetime(2026, 8, 6, 4, 1, 26, tzinfo=timezone.utc)
...
def testparse_bandcamp_date_idempotent_on_datetime():
    dt = datetime(2026, 8, 6, 4, 1, 26, tzinfo=timezone.utc)
...
def test_collection_item_parses_dates_to_datetimes():
    ...
    assert item.added == datetime(2026, 8, 6, 4, 1, 26, tzinfo=timezone.utc)
    assert item.purchased == datetime(2026, 8, 6, 4, 1, 27, tzinfo=timezone.utc)
...
def test_download_item_parses_release_and_sold_dates():
    ...
    assert item.sold_date == datetime(2026, 8, 6, 4, 1, 26, tzinfo=timezone.utc)
    assert item.release_date == datetime(2026, 8, 6, tzinfo=timezone.utc)
...
def test_real_collection_items_response_validates():
    ...
    assert first.added.tzinfo is timezone.utc
